...
from datapipelines.pidata.PIconnect._operators import OPERATORS, add_operators
from datapipelines.pidata.PIconnect._utils import classproperty
from datapipelines.pidata.PIconnect.AFSDK import AF
from datapipelines.pidata.PIconnect.PIConsts import AuthenticationMode
from datapipelines.pidata.PIconnect.PIData import PISeriesContainer
from datapipelines.pidata.PIconnect.time import timestamp_to_index
...
class PIServer(object):  # pylint: disable=useless-object-inheritance
    ...
    def search_list(self, query, source=None):
        """search
        ...
        """
        dot_array=Array[String](query)
        # print(dot_array)
        query_array=AF.PI.PIPoint.FindPIPoints(self.connection,dot_array)
        # print(query_array)
        # print(type(query_array[0]))
        pi_points = AF.PI.PIPointList(query_array)
        # print(pi_points)
        # print(type(pi_points))
        # print(len(pi_points))
        # pi_points_lis=AF.PI.PIPointList(pi_points)
        # print(pi_points_lis)
        # print(type(pi_points_lis))
        # This list comprehension is the part where we actually GET data
        # from the server.
        # !!! perhaps return this as a pandas dataframe.
        #out = [PIPoint(pi_point) for pi_point in pi_points]
        return pi_points
    ...
    def search(self, query, source=None):
        ...
        pi_points = AF.PI.PIPoint.FindPIPoints(self.connection, BuiltinStr(query), source, None)
        # This list comprehension is the part where we actually GET data
        # from the server.
        # !!! perhaps return this as a pandas dataframe.
        out = [PIPoint(pi_point) for pi_point in pi_points]
